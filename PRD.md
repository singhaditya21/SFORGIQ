...
- **v0.9** — **Honesty audit, second pass — and this time the document was understating the code.** v0.8 corrected claims that ran ahead of the repository; a year of building then pushed several of them the other way, and a spec that under-reports is no more usable than one that over-reports. Corrected: free-tool ingestion, described in §7.3 as roadmap-only with "no parser and no invocation for any of the three exists", is fully implemented in `scanner/external.py` including CLI invocation, overlap merging and score exclusion; Org mode, described as a label carrying no org introspection, is implemented in `scanner/org_mode.py` and is what lets D2 run at all, on real record probes. Added as implemented since v0.8: cross-org drift (`scanner/drift.py`), reported outside D1–D5 so it never penalises a score; persona capability surfaces (`scanner/persona.py`, `OrgIQ_Persona__c`), which read profiles, layouts, flows, approvals and validation rules together; finding survival (`scanner/lifecycle.py`); an evidence-responsive effort model with an explicit calibration route (`scanner/effort.py`, `salesforce/calibration_kit.py`); and the rubric extracted to data (`scanner/rubric.json`) so the judgement half can be tuned, and ported, without the engine. The effort model remains **uncalibrated** and says so wherever it appears — that has not changed and is not a defect.
...
**Org mode — [implemented], `scanner/org_mode.py`.** Connects read-only to a live org through the `sf` CLI and collects field schema, Flows, Apex, triggers, permission sets, reports and dashboards, and **record-level probes** — row counts, fill rates, staleness, and a duplicate probe where a groupable key exists. The record probes are what make D2 assessable at all: no repository carries them, so in source mode that dimension is correctly Not Assessed rather than scored on nothing.

What a probe could not measure is recorded rather than defaulted. An org whose Name field is an auto-number has nothing for the duplicate probe to group on; that is reported as an unavailable sub-signal with the collector's own reason, D2 lands at partial coverage, and the dimension is held out of the composite instead of scoring well on a measurement that never happened.

Still [target]: the full dependency graph via `MetadataComponentDependency`, and usage signals from Setup Audit Trail.
...
**Free-tool ingestion — [implemented], `scanner/external.py`.** All three are wired up. Code Analyzer results are parsed in v4, v5 and SARIF shapes, and the CLI can be invoked directly when the plugin is installed; Security Health Check is read from the scanned org; an Optimizer export is parsed from file. Each is optional and each records whether it actually ran, so a tool that is absent produces "not collected" rather than a silently clean result.

This sentence previously read "roadmap-only … no parser and no invocation for any of the three exists", which stopped being true and stayed in the document — the exact failure the status tags were introduced to prevent, running in the opposite direction. A specification that under-reports the code is as unusable as one that over-reports it: a reader cannot tell which sentences to trust either way.

**Provenance is what keeps the ingestion additive.** Every finding carries a `Source` naming the engine that raised it — `Source__c` on `OrgIQ_Finding__c`, `source` in the scan JSON, a `Source` column in the backlog CSV — defaulting to `OrgIQ` for the native rules. Ingested findings join the same record set, the same §4.6 gate and the same backlog without a schema change, and a reviewer can see which engine made a claim before trusting it. Where an external finding overlaps one of OrgIQ's own, the two are merged rather than both emitted, so importing a second opinion never inflates the backlog; and an imported finding is reported and ticketed but does not redefine what a dimension measures, so it is excluded from the dimension score.
